...
class AssetDisposalMasterAPIController extends AppBaseController
{
    ...
    public function __construct(AssetDisposalMasterRepository $assetDisposalMasterRepo, VatReturnFillingMasterRepository $vatReturnFillingMasterRepo)
    {
    ...
    public function amendAssetDisposalReview(Request $request)
    {
        ...
        try {
            ...
            if($masterData->disposalType == 6){
                $customerInvoice = CustomerInvoiceDirect::where('customerinvoiceno', $masterData->disposalDocumentCode)
                ...
                if (!empty($customerInvoice)) {
                    ...
                    //deleting from approval table
                    DocumentApproved::where('documentSystemCode', $customerInvoice->custInvoiceDirectAutoID)
                        ->where('companySystemID', $customerInvoice->companySystemID)
                        ->where('documentSystemID', $customerInvoice->documentSystemiD)
                        ->delete();

                    //deleting from general ledger table
                    GeneralLedger::where('documentSystemCode', $customerInvoice->custInvoiceDirectAutoID)
                        ->where('companySystemID', $customerInvoice->companySystemID)
                        ->where('documentSystemID', $customerInvoice->documentSystemiD)
                        ->delete();

                    //deleting records from accounts receivable
                    AccountsReceivableLedger::where('documentCodeSystem', $customerInvoice->custInvoiceDirectAutoID)
                        ->where('companySystemID', $customerInvoice->companySystemID)
                        ->where('documentSystemID', $customerInvoice->documentSystemiD)
                        ->delete();

                    //deleting records from tax ledger
                    TaxLedger::where('documentMasterAutoID', $customerInvoice->custInvoiceDirectAutoID)
                        ->where('companySystemID', $customerInvoice->companySystemID)
                        ->where('documentSystemID', $customerInvoice->documentSystemiD)
                        ->delete();


                    $taxLedgerDetails = TaxLedgerDetail::where('documentMasterAutoID', $customerInvoice->custInvoiceDirectAutoID)
                        ->where('companySystemID', $customerInvoice->companySystemID)
                        ->where('documentSystemID', $customerInvoice->documentSystemiD)
                        ->get();
                    ...
                    // updating fields
                    $customerInvoice->confirmedYN = 0;
                    $customerInvoice->confirmedByEmpSystemID = null;
                    $customerInvoice->confirmedByEmpID = null;
                    $customerInvoice->confirmedByName = null;
                    $customerInvoice->confirmedDate = null;
                    $customerInvoice->RollLevForApp_curr = 1;

                    $customerInvoice->approved = 0;
                    $customerInvoice->approvedByUserSystemID = null;
                    $customerInvoice->approvedByUserID = null;
                    $customerInvoice->approvedDate = null;
                    $customerInvoice->postedDate = null;
                    $customerInvoice->save();

                    AuditTrial::createAuditTrial($customerInvoice->documentSystemiD,$customerInvoice->custInvoiceDirectAutoID,$input['returnComment'],'returned back to amend');

                    // delete customer invoice details
                    CustomerInvoiceDirectDetail::where('custInvoiceDirectID',$customerInvoice->custInvoiceDirectAutoID)->delete();

                    // cancel customer invoice
                    $employee = UserTypeService::getSystemEmployee();

                    $customerInvoice->canceledYN = -1;
                    $customerInvoice->canceledComments = $request['cancelComments'];
                    $customerInvoice->canceledDateTime = NOW();
                    $customerInvoice->canceledByEmpSystemID = $employee->employeeSystemID;
    ...
    public function checkDisposalAmendReview(Request $request)
    {
        $input = $request->all();
        ...
        if($masterData->disposalType == 6){
            ...
            if (!empty($customerInvoice)) {
                $checkReceiptVoucherAvailable = CustomerReceivePaymentDetail::where('bookingInvCodeSystem', $customerInvoice->custInvoiceDirectAutoID)
                    ->where('companySystemID', $customerInvoice->companySystemID)
                    ->where('addedDocumentSystemID', $customerInvoice->documentSystemiD)
                    ->first();
                ...
                if(empty($checkReceiptVoucherAvailable)) {
                    ...
                    return $this->sendError(
                        'Generated customer invoice through disposal has been pulled into the receipt voucher',
                        404,
                        array('type' => 1)
                    );
                }
