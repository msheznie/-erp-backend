...
class DocumentApprovedAPIController extends AppBaseController
{
    ...
    public function getAllDocumentApproval(request $request)
    {
		...
		if(isset($input['documentSystemCode']) && $input['documentSystemCode'] != ''){
			$filter .= " AND erp_documentapproved.documentSystemCode = " . $input['documentSystemCode'];
		}
        ...
        if($isApproved){
			...
			FROM
				...
				AND erp_purchaserequest.RollLevForApp_curr = erp_documentapproved.rollLevelOrder
				AND (
					(companydocumentattachment.isServiceLineApproval = -1 AND erp_purchaserequest.serviceLineSystemID = erp_documentapproved.serviceLineSystemID)
					OR companydocumentattachment.isServiceLineApproval IS NULL
					OR companydocumentattachment.isServiceLineApproval != -1
				) 
				LEFT JOIN (
		...
		if(isset($input['documentSystemCode']) && $input['documentSystemCode'] != ''){
			return $this->sendResponse(['validated' => count($output) > 0 ? 1 : 0],trans('custom.data_retrived_successfully_1'));
		}
