    ...
    public function getSupplierInvoiceTaxDetailsQuery($request)
    {
        ...
        $query = '
            ...
            SELECT
                ...
                IFNULL((invoiceMaster.bookingAmountRpt), 0) AS bookingAmountRpt,
                (SUM(details.supplierInvoAmount/(ep.unitCost - ep.discountAmount + ep.VATAmount) * ep.VATAmount) - (SUM(IFNULL((details.exempt_vat_portion)/100,0) * (details.supplierInvoAmount/(ep.unitCost - ep.discountAmount + ep.VATAmount) * ep.VATAmount)) - IFNULL(invoiceMaster.retentionVatAmount,0))) AS taxTotalAmount,
                IFNULL((invoiceMaster.retentionAmount - invoiceMaster.retentionVatAmount),0) AS retentionAmount,      
                details.vatSubCategoryID,
