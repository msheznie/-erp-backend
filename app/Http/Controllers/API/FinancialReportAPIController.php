...
AND epsim .invoiceType = 3 AND taxTotalAmount > 0';
    ...
    public function getSupplierInvoiceTaxDetailsQuery($request)
    {
        ...
        $query = '
            ...
            SELECT
                ...
                (SUM(details.VATAmount * details.noQty) - (IFNULL(SUM(details.exempt_vat_portion)/100,0) * SUM(details.VATAmount * details.noQty)) - IFNULL(invoiceMaster.retentionVatAmount,0)) AS taxTotalAmount,
                IFNULL((invoiceMaster.retentionAmount - invoiceMaster.retentionVatAmount),0) AS retentionAmount,
                details.vatSubCategoryID,
    ...
    IFNULL((invoiceMaster.bookingAmountRpt), 0) AS bookingAmountRpt,
            ...
            SELECT
                ...
                IFNULL(SUM((details.supplierInvoAmount/(ep.unitCost - ep.discountAmount + ep.VATAmount)) * ep.unitCost), 0) AS bookingAmountTrans,
                IFNULL((invoiceMaster.bookingAmountRpt), 0) AS bookingAmountRpt,
                (SUM(details.supplierInvoAmount/(ep.unitCost - ep.discountAmount + ep.VATAmount) * ep.VATAmount) - (IFNULL(SUM(details.exempt_vat_portion)/100,0) * SUM(details.supplierInvoAmount/(ep.unitCost - ep.discountAmount + ep.VATAmount) * ep.VATAmount)) - IFNULL(invoiceMaster.retentionVatAmount,0)) AS taxTotalAmount,
                IFNULL((invoiceMaster.retentionAmount - invoiceMaster.retentionVatAmount),0) AS retentionAmount,      
                details.vatSubCategoryID,
