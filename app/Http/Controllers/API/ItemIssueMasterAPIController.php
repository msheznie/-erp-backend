...
class ItemIssueMasterAPIController extends AppBaseController
{
    ...
    public function update($id, UpdateItemIssueMasterAPIRequest $request)
    {
        ...
        if($itemIssueMaster->mfqJobID)
        {
            ...
            $client = new Client();
            $res = $client->request('GET', env('HOST_URL').'/getJobStatus?JobID='.$itemIssueMaster->mfqJobID, [
                'headers' => [
    ...
    public function getMaterielIssueFormData(Request $request)
    {
        ...
        $client = new Client();
        $res = $client->request('GET', env('HOST_URL').'/getOpenJobs?company_id'.$companyId.'=&warehouse=&segment=', [
            'headers' => [
            'Content-Type'=> 'application/json',
            'token' => $hashKey,
            'api_key' => $api_key
