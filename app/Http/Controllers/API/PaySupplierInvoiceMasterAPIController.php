...
class PaySupplierInvoiceMasterAPIController extends AppBaseController
{
    ...
    public function update($id, UpdatePaySupplierInvoiceMasterAPIRequest $request)
    {
        ...
        try {
            ...
            if ($pvUpdateStatus['status']) {
                DB::commit();
                return $this->sendReponseWithDetails($pvUpdateStatus['data'], $pvUpdateStatus['message'],1,$pvUpdateStatus['additionalData'] ?? null);
            }
            else {
                DB::rollBack();
                return $this->sendError(
                    $pvUpdateStatus['message'],
                    isset($pvUpdateStatus['code']) && ($pvUpdateStatus['code'] != 404) ? $pvUpdateStatus['code'] : 404,
                    $pvUpdateStatus['type'] ?? array('type' => '')
                );
            }
        } catch
        (\Exception $exception) {
...
AND MASTER.companySystemID = ' . $input['companySystemID'] . ' AND BPVsupplierID = ' . $input['BPVsupplierID'] . ' HAVING (ROUND(BalanceAmt, currency.DecimalPlaces) > 0)');
    ...
    public function createPaymentVoucherAPI(Request $request){
        ...
        if(is_array($pvs)) {
            ...
            if ($company) {
                if ($company->vatRegisteredYN == 1) {
                    $vatData = Tax::where('companySystemID', $compId)->where('isActive', 1)->where('taxCategory', 2)->first();
                    if ($vatData) {
                        $subCategoryExists = TaxVatCategories::where('taxMasterAutoID',$vatData->taxMasterAutoID)->where('subCatgeoryType', 1)->exists();
                        if (!$subCategoryExists) {
                            return $this->sendError("VAT subcategory not created");
                        }
                    }
                    else {
                        return $this->sendError("VAT setup is not created for this company");
                    }
                }
                else {
                    return $this->sendError("VAT is not registered for this company.");
                }
            }
            else {
