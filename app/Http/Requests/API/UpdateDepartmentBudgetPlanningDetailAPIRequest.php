...
class UpdateDepartmentBudgetPlanningDetailAPIRequest extends FormRequest
{
    /**
    ...
    {
        $id = $this->route('departmentBudgetPlanningDetail');
        
        return [
            'department_planning_id' => 'sometimes|required|integer|exists:department_budget_plannings,id',
            'department_segment_id' => 'nullable|integer|exists:company_departments_segments,departmentSegmentSystemID',
            'budget_template_gl_id' => 'sometimes|required|integer|exists:dep_budget_template_gl,depBudgetTemplateGlID',
            'request_amount' => 'sometimes|required|numeric|min:0',
            'responsible_person' => 'nullable|integer|exists:users,id',
            'responsible_person_type' => 'sometimes|required|integer|in:1,2',
