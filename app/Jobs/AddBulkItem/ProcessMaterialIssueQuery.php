...
use App\Jobs\AddBulkItem\ProcessMaterialIssueBulk;
...
class ProcessMaterialIssueQuery implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $itemMasters = ItemMaster::whereHas('itemAssigned', function ($query) use ($companyId) {
                                     ...
                                     ->whereDoesntHave('material_issue_details', function($query) use ($requestID) {
                                        $query->where('itemIssueAutoID', $requestID);
                                     })
