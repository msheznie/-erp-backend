        ...
        if($depMaster && !$depMaster->is_acc_dep) {
            ...
            try {
                ...
                if($depMaster) {
                    ...
                    if (count($faMaster) > 0) {
                        $chunkSize = 1000;
                        $faMaster->chunk($chunkSize)->each(function ($chunk) use ($db, $depMasterAutoID, $depMaster, $depDate) {
                            DepreciationSubJobs::dispatch($db, $chunk->all(), $depMasterAutoID, $depMaster, $depDate);
                        });
                    }
