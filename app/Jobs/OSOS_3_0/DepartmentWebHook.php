...
use App\Services\OSOS_3_0\DepartmentService;
...
class DepartmentWebHook implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $service = new DepartmentService($this->dataBase, $this->id, $this->postType, $this->thirdPartyData);
        $resp = $service->execute();

        if(isset($resp['status']) && !$resp['status']){
            $this->callDepartmentService($resp['code'], 'Department');
        }
    }

    function callDepartmentService($statusCode, $desc)
    {
