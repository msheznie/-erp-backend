...
use App\Services\OSOS_3_0\ExperienceService;
...
class ExperienceWebHook implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $service = new ExperienceService($this->dataBase, $this->id, $this->postType, $this->thirdPartyData);
        $resp = $service->execute();

        if(isset($resp['status']) && !$resp['status']){
            $this->callExperienceService($resp['code'], 'Experience');
        }
    }

    function callExperienceService($statusCode, $desc)
    {
        if (!in_array($statusCode, [200, 201])) {
            for ($i = 1; $i <= 3; $i++) {
                $logData = ['message' => 'Api Experience attempt'. $i, 'id' => $this->id ];
