...
use App\Services\OSOS_3_0\FieldOfStudyService;
...
class FieldOfStudyWebHook implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $service = new FieldOfStudyService($this->dataBase, $this->id, $this->postType, $this->thirdPartyData);
        $resp = $service->execute();

        if(isset($resp['status']) && !$resp['status']){
            $this->callFieldOfStudyService($resp['code'], 'Field of Study');
        }
    }

    function callFieldOfStudyService($statusCode, $desc)
    {
        if (!in_array($statusCode, [200, 201])) {
            for ($i = 1; $i <= 3; $i++) {
                $logData = ['message' => 'Api Field of Study attempt'. $i, 'id' => $this->id ];
                $this->insertToLogTb($logData, 'info', $desc, $this->thirdPartyData['company_id']);

                $service = new FieldOfStudyService(
