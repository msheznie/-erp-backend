...
class CreditNoteRepository extends BaseRepository
{
    ...
    public function creditNoteListQuery($request, $input, $search = '', $customerID = null, $projectID = null) {
        ...
        $master->select('creditNoteCode','erp_creditnote.postedDate' ,'CurrencyCode', 'erp_creditnote.approvedDate', 'creditNoteDate', 'erp_creditnote.comments', 'empName', 'DecimalPlaces', 'erp_creditnote.confirmedYN', 'erp_creditnote.approved', 'erp_creditnote.refferedBackYN', 'erp_creditnote.canceledYN', 'creditNoteAutoID', 'customermaster.CutomerCode', 'customermaster.CustomerName', 'creditAmountTrans', 'erp_projectmaster.description as project_description', DB::raw('CASE WHEN document_system_mapping.documentId IS NOT NULL THEN 1 ELSE 0 END as AutoGenerated'));
    ...
    public function setExportExcelData($dataSet) {
        ...
        if (count($dataSet) > 0) {
            ...
            foreach ($dataSet as $val) {
                ...
                $data[$x][trans('custom.status')] = StatusService::getStatus($val->canceledYN ?? 0, NULL, $val->confirmedYN, $val->approved, $val->refferedBackYN);
