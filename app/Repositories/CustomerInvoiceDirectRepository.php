...
class CustomerInvoiceDirectRepository extends BaseRepository
{
    ...
    $invMaster->select(
        ...
        DB::raw('COALESCE(
            CASE 
                WHEN erp_custinvoicedirect.isPerforma IN (0,1) THEN (
                    SELECT SUM(erp_custinvoicedirectdet.VATAmount * erp_custinvoicedirectdet.invoiceQty)
                    FROM erp_custinvoicedirectdet
                    WHERE erp_custinvoicedirectdet.custInvoiceDirectID = erp_custinvoicedirect.custInvoiceDirectAutoID
                )
                WHEN erp_custinvoicedirect.isPerforma IN (2,3,4,5) THEN (
                    SELECT COALESCE(SUM(erp_customerinvoiceitemdetails.VATAmount * erp_customerinvoiceitemdetails.qtyIssued), 0)
                    FROM erp_customerinvoiceitemdetails
                    WHERE erp_customerinvoiceitemdetails.custInvoiceDirectAutoID = erp_custinvoicedirect.custInvoiceDirectAutoID
                )
                ELSE 0
            END, 0
        ) as vatAmountTotal')
    ...
    public function setExportExcelData($dataSet) {
        ...
        if (count($dataSet) > 0) {
            ...
            foreach ($dataSet as $val) {
                ...
                $data[$x][trans('custom.vat')] = $val->vatAmountTotal? number_format($val->vatAmountTotal,  $val->DecimalPlaces, ".", "") : '';
