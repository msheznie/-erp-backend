    ...
    public function paySupplierInvoiceListQuery($request, $input, $search = '', $supplierID = null, $projectID = null, $employeeID = null,$createdBy = null,$customerID = null) {
        ...
        $paymentVoucher = PaySupplierInvoiceMaster::with(['customer', 'supplier', 'created_by', 'suppliercurrency', 'bankcurrency', 'expense_claim_type', 'paymentmode', 'project','pdc_cheque','localcurrency','rptcurrency'])
                        ...
                        ->leftJoin('document_system_mapping', function ($join) {
                            $join->on('document_system_mapping.documentId', '=', 'erp_paysupplierinvoicemaster.PayMasterAutoId')
                                 ->where('document_system_mapping.documentSystemId', '=', 4);
                        })
                        ...
                        ->addSelect(DB::raw('CASE WHEN document_system_mapping.documentId IS NOT NULL THEN 1 ELSE 0 END as AutoGenerated'))
