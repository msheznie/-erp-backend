    ...
    public function setExportExcelData($dataSet) {
        ...
        if (count($dataSet) > 0) {
            ...
            foreach ($dataSet as $val) {
                $data[$x][trans('custom.e_doc_code')] = $val->stockAdjustmentCode;
                $data[$x][trans('custom.e_segment')] = $val->segment_by? $val->segment_by->ServiceLineDes : '';
                $data[$x][trans('custom.e_reference_no')] = $val->refNo;
                $data[$x][trans('custom.e_date')] = \Helper::dateFormat($val->stockAdjustmentDate);
                $data[$x][trans('custom.e_location')] = $val->warehouse_by? $val->warehouse_by->wareHouseDescription : '';
