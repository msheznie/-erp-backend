        ...
        if ($masterData) {
                ...
                if ($masterData->documentType != 4) {
                    ...
                    if ($masterData->documentType == 3) {
                        ...
                        if (!TaxService::isSupplierInvoiceRcmActivated($masterModel["autoID"])) {
                            $retentionTransWithoutVat = ($data['documentTransAmount'] + (ABS($totalVATAmount) - ($excemptVATAmount - $standardExemptVATAmount))) * ($retentionPercentage / 100);
                            $retentionLocalWithoutVat = ($data['documentLocalAmount'] + (ABS($totalVATAmountLocal) - ($excemptVATAmountLocal - $standardExemptVATAmountLocal))) * ($retentionPercentage / 100);
                            $retentionRptWithoutVat = ($data['documentRptAmount'] + (ABS($totalVATAmountRpt) - ($excemptVATAmountRpt - $standardExemptVATAmountRpt))) * ($retentionPercentage / 100);
                        } else {
                            $retentionTrans = $data['documentTransAmount'] * ($retentionPercentage / 100);
                            $retentionLocal = $data['documentLocalAmount'] * ($retentionPercentage / 100);
                            $retentionRpt = $data['documentRptAmount'] * ($retentionPercentage / 100);
            ...
            if ($masterData->documentType == 0 || $masterData->documentType == 2) {
                ...
                if(!empty($exemptExpenseDetails) && !empty($expenseCOA) && $expenseCOA->expenseGL != null && $exemptExpenseDetails->VATAmount != 0){
                    ...
                    $data['serviceLineCode'] = SegmentMaster::getSegmentCode($masterData->serviceLineSystemID);
                ...
                if ($masterData->rcmActivated != 1) {
                $exemptExpenseDetails = TaxService::processSIExemptVatDirectInvoice($masterModel["autoID"], true);
                $expenseCOA = TaxVatCategories::with(['tax'])->where('subCatgeoryType', 3)->whereHas('tax', function ($query) use ($masterData) {
                    $query->where('companySystemID', $masterData->companySystemID);
                })->where('isActive', 1)->first();
                    if(!empty($exemptExpenseDetails) && !empty($expenseCOA) && $expenseCOA->expenseGL != null) {

                        foreach ($exemptExpenseDetails as $key => $value) {
