...
class BudgetConsumptionService
{
	...
	public static function getActualConsumption($companyId, $financialYear, $glAccount, $glType, $categoryBLorPL) {

		$chartOfAccountIDs = BudgetConsumedData::where('companySystemID', $companyId)
				->where('companyFinanceYearID', $financialYear)
				->whereHas('chart_of_account', function ($query) use ($categoryBLorPL) {
					$query->where('catogaryBLorPLID', $categoryBLorPL);
				})
				->distinct()
		...
		if($glType == 'profit_loss') {
			$results = self::actualConsumptionPandL($companyId, $chartOfAccountIDs, $financialYear, $glAccount);
		} else {
			$results = self::actualConsumptionBL($companyId, $chartOfAccountIDs, $financialYear, $glAccount);
		}
					...
					$query->select('grvDetailsID', 'grvAutoID', 'purchaseOrderMastertID',
						...
						->with(['grv_master' => function ($query) {
							$query->with('details')->select('grvAutoID', 'grvPrimaryCode', 'approved',
								'grvConfirmedYN', 'grvTotalComRptCurrency');
						}]);
				}]);
			}])
			->where('consumeYN', -1)
