...
function EAN13($x, $y, $barcode, $h=16, $w=.35, $fSize=9,$bool,$price,$name)
{
    $this->Barcode($x,$y,$barcode,$h,$w,$fSize,13,$bool,$price,$name);
}
...
function Barcode($x, $y, $barcode, $h, $w, $fSize, $len,$bool,$price,$name)
{
    // $this->SetFont('Arial','',$fSize);
    // $this->Text($x,$y-22+$h+11.2,'Product Name : '.$name);
    ...
    // if($bool)
    // {
    //     $this->Text($x,$y-2.5+$h+11,$price.'.00/=');
    // }
...
function Code39($assetData, $ext = true, $cks = false, $w = 0.4, $h = 20, $wide = true) {
   ...
   //  $this->Text($x-80,$y+$h+10,$price.'.00/=');
    ...
    if($ext) {
        ...
        // $code = $this->encode_code39_ext($price);
    }
    else {
...
function Code39_2($xpos, $ypos, $code, $baseline=0.5, $height=5,$is_price,$price){
    ...
    {
        $this->SetFont('Arial','',10);
        $this->Text($xpos, $ypos + $height + 8, '$: '.$price.'.00');
        $this->SetFillColor(0);
    }
