...
var AppSidebarNavComponent = /** @class */ (function () {
    function AppSidebarNavComponent(restangular) {
    ...
    AppSidebarNavComponent.prototype.ngOnInit = function () {
        var _this = this;
        var baseAccounts = this.restangular.one('navigation_user_group_setups');
        baseAccounts.getList().subscribe(function (menu) {
            for (var i = 0; i < menu.length; i++) {
                var modal = {
                    name: menu[i].description,
                    url: '/' + menu[i].url,
                    icon: menu[i].pageIcon,
                    badge: {
                        variant: '',
                        text: ''
                    }
                };
                for (var k = 0; k < menu[i].child.length; k++) {
                    if (k == 0) {
                        modal.children = [];
                    }
                    var child = menu[i].child[k];
                    var temChild = {
                    ...
                    for (var n = 0; n < child.child.length; n++) {
                        if (n == 0) {
                            temChild.children = [];
                        }
                        var sec_child = child.child[n];
                        temChild.children.push({
                            name: sec_child.description,
                            url: '/' + sec_child.url,
                            icon: sec_child.pageIcon,
                            badge: {
                                variant: '',
                                text: ''
                            }
                    ...
                    modal.children.push(temChild);
                }
                _this.navigation.push(modal);
                debugger;
            }
        }, function () {
