<!-- Idpoaddons Field -->
<div class="form-group col-sm-6">
    {!! Form::label('idpoAddons', 'Idpoaddons:') !!}
    {!! Form::number('idpoAddons', null, ['class' => 'form-control']) !!}
</div>

<!-- Poid Field -->
<div class="form-group col-sm-6">
    {!! Form::label('poId', 'Poid:') !!}
    {!! Form::number('poId', null, ['class' => 'form-control']) !!}
</div>

<!-- Idaddoncostcategories Field -->
<div class="form-group col-sm-6">
    {!! Form::label('idaddOnCostCategories', 'Idaddoncostcategories:') !!}
    {!! Form::number('idaddOnCostCategories', null, ['class' => 'form-control']) !!}
</div>

<!-- Supplierid Field -->
<div class="form-group col-sm-6">
    {!! Form::label('supplierID', 'Supplierid:') !!}
    {!! Form::number('supplierID', null, ['class' => 'form-control']) !!}
</div>

<!-- Currencyid Field -->
<div class="form-group col-sm-6">
    {!! Form::label('currencyID', 'Currencyid:') !!}
    {!! Form::number('currencyID', null, ['class' => 'form-control']) !!}
</div>

<!-- Amount Field -->
<div class="form-group col-sm-6">
    {!! Form::label('amount', 'Amount:') !!}
    {!! Form::number('amount', null, ['class' => 'form-control']) !!}
</div>

<!-- Glcode Field -->
<div class="form-group col-sm-6">
    {!! Form::label('glCode', 'Glcode:') !!}
    {!! Form::text('glCode', null, ['class' => 'form-control']) !!}
</div>

<!-- Timesreferred Field -->
<div class="form-group col-sm-6">
    {!! Form::label('timesReferred', 'Timesreferred:') !!}
    {!! Form::number('timesReferred', null, ['class' => 'form-control']) !!}
</div>

<!-- Timestamp Field -->
<div class="form-group col-sm-6">
    {!! Form::label('timestamp', 'Timestamp:') !!}
    {!! Form::date('timestamp', null, ['class' => 'form-control']) !!}
</div>

<!-- Submit Field -->
<div class="form-group col-sm-12">
    {!! Form::submit('Save', ['class' => 'btn btn-primary']) !!}
    <a href="{!! route('poAddonsRefferedBacks.index') !!}" class="btn btn-default">Cancel</a>
</div>
