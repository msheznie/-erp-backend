        ...
        <table class="table table-bordered table-striped table-sm">
            ...
            @foreach ($podata->detail as $det)
                ...
                <tr style="border-bottom: 1px solid black; width: 100%;  background-color:#f9f9f9">
                    ...
                    <td class="text-right">{{!is_null($det->unit) && !is_null($det->unit->displayRoundOff) ? number_format($det->noQty, $det->unit->displayRoundOff, '.', '') : number_format($det->noQty, 5, '.', '')}}</td>
                    @if(isset($allowAltUom))
                    <td>
                       @if($det->altUom)
                    ...
                    @endif
                    <td class="text-right">{{!is_null($det->unit) && !is_null($det->unit->displayRoundOff) ? number_format($det->unitCost, $det->unit->displayRoundOff, '.', '') : number_format($det->unitCost, 5, '.', '')}}</td>
                    <td class="text-right">{{!is_null($det->unit) && !is_null($det->unit->displayRoundOff) ? number_format($det->discountAmount, $det->unit->displayRoundOff, '.', '') : number_format($det->discountAmount, 5, '.', '')}}</td>
                    @if ($podata->isVatEligible)
                    ...
                    @endif
                    <td class="text-right">{{!is_null($det->unit) && !is_null($det->unit->displayRoundOff) ? number_format($netUnitCost, $det->unit->displayRoundOff, '.', '') : number_format($netUnitCost, 5, '.', '')}}</td>
                    <td class="text-right">{{!is_null($det->unit) && !is_null($det->unit->displayRoundOff) ? number_format($det->netAmount, $det->unit->displayRoundOff, '.', '') : number_format($det->netAmount, 5, '.', '')}}</td>
                </tr>
                {{ $x++ }}
            @endforeach
